...
const DungeonQuests = () => {
  ...
  const fetchQuests = async () => {
    try {
      ...
      showMessage('Failed to load dungeon quests');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.title.trim()) {
      showMessage('Please enter a completion title');
      return;
    }
    ...
    try {
      const response = await axios.post('https://leveling-system-mern-project.onrender.com/api/dungeon-quests', {
      ...
      if (response.data.success) {
        showMessage('Dungeon quest created successfully!', true);
        // Reset form
        setFormData({
          name: '',
          xp: '',
      ...
      if (error.response?.data?.message) {
        ...
        showMessage('Failed to create dungeon quest');
      }
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDeleteQuest = async (questId) => {
    ...
    try {
      const response = await axios.delete(`https://leveling-system-mern-project.onrender.com/api/dungeon-quests/${questId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-slate-900/70 backdrop-blur-xl rounded-xl shadow-xl border border-purple-600/50 p-6 mb-8 relative overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
              ...
              {/* Completion Title */}
              <div>
                <label htmlFor="title" className="block text-sm font-semibold text-purple-300 mb-2 flex items-center">
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  placeholder="e.g., Master of React JS"
                  className="w-full px-4 py-3 bg-slate-800/80 border border-purple-600/50 rounded-lg text-black placeholder-purple-300 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-400 transition-all duration-300 font-medium shadow-lg backdrop-blur-sm"
                  disabled={submitting}
                  required
                />
                <p className="text-sm text-purple-400 mt-1 font-medium">
                  Title awarded upon quest completion
                </p>
              </div>
          ...
          <div className="bg-slate-900/70 backdrop-blur-xl rounded-xl shadow-xl border border-purple-600/50 p-6 relative overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="text-center py-12 relative z-10">
                ...
                <p className="text-purple-300 text-xl font-semibold">No Quests Created Yet</p>
                <p className="text-purple-400 font-medium">Begin your journey by creating your first quest above!</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
                {quests.map((quest) => (
                  <div
                    key={quest._id}
                    className={`relative p-5 rounded-lg border transition-all duration-300 transform hover:scale-[1.02] shadow-lg backdrop-blur-sm ${
                      quest.isCompleted
                        ? 'bg-emerald-900/30 border-emerald-500/50 shadow-emerald-500/10'
                        : 'bg-slate-800/60 border-purple-600/50 hover:border-cyan-500/70 shadow-purple-500/10'
                    }`}
                  >
                    {/* Quest Controls */}
                    ...
                    <div className="pr-16">
                      <h4 className={`font-bold text-lg mb-3 flex items-center ${
                        quest.isCompleted ? 'text-emerald-400 line-through' : 'text-purple-300'
                      }`}>
                      ...
                      <div className="space-y-2">
                        ...
                        {quest.title && (
                          <div className="flex items-center text-sm font-medium">
                            ...
                            <span className="text-purple-300">{quest.title}</span>
                          </div>
                        )}
