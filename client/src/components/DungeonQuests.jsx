...
const DungeonQuests = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/dungeon-quests', {
  ...
  const handleDeleteQuest = async (questId) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:5000/api/dungeon-quests/${questId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
